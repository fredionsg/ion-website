  ...
  window.addEventListener('scroll', function() {
    ...
    document.querySelectorAll('section[id]').forEach(function(section) {
      ...
      if (
        ...
        const activeLink = document.querySelector('.nav-links a[href="#' + sectionId + '"], .nav-links a[href="/#' + sectionId + '"]');
...
form.addEventListener('submit', function (e) {
  ...
  // Allow POST to Google (do not preventDefault)
...
/**********************************************************
 * SECTION 6: Visual polish helpers (reveal + scrolled nav)
 **********************************************************/
document.addEventListener("DOMContentLoaded", function () {
  ...
  const revealTargets = document.querySelectorAll(
    "section, .team-member, .service-card, .vision-mission-card, .support-category, .education-item, .step, .cc-row, .wp-row, .cta-option"
  );
